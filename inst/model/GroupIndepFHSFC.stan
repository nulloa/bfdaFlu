...
parameters {
  ...
  vector<lower=0>[N_subj] c2; // SD for beta or Global Shrinkage Param
...
transformed parameters {
  ...
  vector<lower=0>[dim_space] lamb_tilde[N_subj]; // lambda in beta ~ N(0, lambda_tilde*tau)
    
  for(s in 1:N_subj){
    for(j in 1:dim_space){
      lamb_tilde[s,j] = sqrt( c2[s] * square(lamb[s,j]) ./ (c2[s] + square(tau[s]) * square(lamb[s,j])) );
    }
  }
...
model {
  ...
  for(s in 1:N_subj){
    for(j in 1:dim_space){
      lamb[s,j] ~ cauchy(0,1); //Prior on  SD
    }
  }
  ...
  for(i in 1:N_subj){
    for (j in 1:dim_space){
      beta[i,j] ~ normal(0, lamb_tilde[i,j]*tau[i]);
	  }
