#' cauchy_bfda_fc
#' 
#' Runs BFDA with horseshoe prior on betas and cauchy prior on sd for forecasting
#'
...
cauchy_bfda_fc <- function(W, E, model="Indep", group=NULL, niter=4000, nwarmup=niter/2, nchains=3, thin=1, inits=NULL){
  # Load Library
  require(rstan)
  ...
  model.name <- NULL
  if(model=="Indep"){
    model.name <- "IndepHorseshoeFC.stan"
  }else if(model=="Group Indep"){
    model.name <- "CauchyGroupIndepHorseshoeFC.stan"
  }else if(model=="Group Hier"){
    model.name <- "CauchyGroupHierHorseshoeFC.stan"
  }else{
