...
#' @param model Which hierarchy structure to be used: Indep (default), Region, Season
...
horseshoe_bfda <- function(W, E, model="Indep", group=NULL, niter=4000, nwarmup=niter/2, nchains=3, thin=1, inits=NULL){
  ...
  model.name <- NULL
  if(model=="Indep"){
    model.name <- "IndepHorseshoeBFDA.stan"
  }else if(model=="Region" | model=="Season"){
    model.name <- "GroupHorseshoeBFDA.stan"
  }else{
    cat("Misspecified model. Model needs to be 'Indep', 'Region', or 'Season'")
    stop()
  }
