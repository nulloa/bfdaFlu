...
cauchy_bfda <- function(W, E, model="Indep", group=NULL, niter=4000, nwarmup=niter/2, nchains=3, thin=1, inits=NULL){
  # Load Library
  require(rstan)
  ...
  if(model=="Indep"){
    ...
    model.name <- "CauchyGroupIndepHorseshoe.stan"
  }else if(model=="Group Hier"){
    model.name <- "CauchyGroupHierHorseshoe.stan"
  }else{
